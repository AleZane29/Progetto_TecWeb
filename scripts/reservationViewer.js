...
function filterTable() {
	...
	rows.forEach((row) => {
		const sport = row.cells[0].textContent.toLowerCase();
		const court = row.cells[1].textContent.toLowerCase();
		...
		if (matchSport && matchCourt && matchDate) {
			row.style.display = '';
		} else {
...
function confirmEdit() {
    ...
    // Aspetto per vedere se c'è errore di sovrapposizione
	return false; // Preveniamo il refresh del form
}
...
function changeDateDialog() {
	...
	rows.forEach((row) => {
		if (row !== currentRow) {
			const rowCourt = row.cells[1].textContent;
			const rowSport = row.cells[0].textContent;
			const date = row.cells[2].textContent;
